  ...
  // const API_BASE = "https://66624fa4-5560-4fef-96c2-224a50172da8-00-inci5z2l9nqr.picard.replit.dev";
   ...
   * Se l'utente è loggato -> tab Profile, Sell, Dungeon, Complete, Logout
   */
  function buildTabs() {
    ...
    if (!user) {
      ...
      let contentHTML = `
        ...
        <div class="sl-tab-content" id="register" style="text-align: left;">
          <p><strong>Registrazione Utente</strong></p>
          <label>User ID (opzionale)</label>
          ...
          <label>Avatar URL (opzionale)</label>
          <input type="text" id="sl-reg-avatar" class="sl-input" placeholder="URL dell'avatar" />
      ...
      document.querySelectorAll("#sl-tab-nav .sl-tab-btn").forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          if (targetElement) {
            ...
            console.error("Elemento con ID '" + target + "' non trovato.");
          }
        });
      });
      ...
      // Utente loggato => tab Profile, Sell, Dungeon, Complete, Logout
      const tabs = [
      ...
      let contentHTML = `
        ...
        <div class="sl-tab-content" id="sell" style="text-align: left;">
          <p>Vendi un oggetto:</p>
          <select id="sl-sell-select" class="sl-input"></select>
          <button class="sl-btn" id="sl-sell-btn">Vendi</button>
          <div id="sl-sell-result" style="margin-top:10px;"></div>
        </div>
  
        <div class="sl-tab-content" id="dungeon" style="text-align: left;">
          <p>Genera un dungeon (E, D, C, F, A, S):</p>
          <input type="text" id="sl-dungeon-diff" class="sl-input" placeholder="Inserisci difficoltà" style="width: 98%;" />
          <button class="sl-btn" id="sl-dungeon-btn">Crea Dungeon</button>
          <div id="sl-dungeon-result" style="margin-top:10px;"></div>
        </div>
  
        <div class="sl-tab-content" id="dungeoncomplete" style="text-align: left;">
        ...
        <div class="sl-tab-content" id="logout" style="text-align: left;">
      ...
      document.getElementById("sl-sell-btn").addEventListener("click", sellItem);
      ...
      // Popola il dropdown degli oggetti da vendere
      fillSellDropdown();
    }
  }
  ...
  async function fillSellDropdown() {
    const user = JSON.parse(localStorage.getItem("slUser"));
    const resp = await callAPI(`/profile/${user.username}`, "GET");
    const sellSelect = document.getElementById("sl-sell-select");
    sellSelect.innerHTML = "";
    if (resp.inventory && resp.inventory.length > 0) {
      resp.inventory.forEach((item) => {
        const option = document.createElement("option");
        option.value = item.name;  // Passa il nome dell'oggetto
        option.textContent = `${item.name} (${item.grade})`;
        sellSelect.appendChild(option);
      });
    } else {
      sellSelect.innerHTML = `<option value="">Inventario vuoto</option>`;
    }
  }
  ...
  async function sellItem() {
    ...
    const sellSelect = document.getElementById("sl-sell-select");
    if (!sellSelect.value) {
      alert("Nessun item selezionato");
      return;
    }
    const itemName = sellSelect.value;
    const body = {
      user_id: Number(user.user_id),
      item_name: itemName,
    };
    const resp = await callAPI("/sell", "POST", body);
    const resultDiv = document.getElementById("sl-sell-result");
    if (resp.error) {
      resultDiv.innerHTML = `<div style="color:red;">Errore: ${resp.error}</div>`;
    } else if (resp.message) {
      resultDiv.innerHTML = `<div>${resp.message}</div>`;
      fillSellDropdown(); // Aggiorna il dropdown
    }
  }
  ...
  // Esporta buildInventoryHtml globalmente se necessario
  window.buildInventoryHtml = buildInventoryHtml;
