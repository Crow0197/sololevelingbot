...
def get_random_drops(count=1, user_grade=None):
    ...
    if user_grade == "B":
        ...
        if rand < 50:
            chosen_grade = random.choice(lower)
        elif rand < 90:
            chosen_grade = "B"
        else:
            chosen_grade = "A"
        filtered = [item for item in all_items if item.get("grade") == chosen_grade]
        if filtered:
...
def generate_dungeon(difficulty):
    ...
    if difficulty == "B":
        ...
        drop_count = random.choices([1, 2, 3], weights=[0.5, 0.4, 0.1])[0]
        money = random.randint(1000, 2000)
        exp_reward = random.randint(200, 400)
    elif difficulty == "A":
        num_miniboss = random.randint(3, 4)
        drop_count = 3
        money = random.randint(2000, 3000)
        exp_reward = random.randint(400, 600)
    elif difficulty == "S":
        num_miniboss = random.randint(4, 6)
        drop_count = 3
        money = random.randint(3000, 5000)
        exp_reward = random.randint(600, 800)
    else:
        return None
    ...
    for _ in range(num_miniboss):
        ...
        if miniboss:
            drops = get_random_drops(drop_count)
            encounters.append({
                "tipo": "miniboss",
                "nemico": miniboss,
                "drops": drops
    ...
    boss_drops = get_random_drops(drop_count)
    encounters.append({
        "tipo": "boss",
        "nemico": main_boss,
        "drops": boss_drops
